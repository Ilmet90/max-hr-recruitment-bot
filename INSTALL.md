...
Пароль главной web-учётной записи, служебный код `/admin` и `MAX_BOT_TOKEN` вводятся обычным видимым текстом. Если пароль или служебный код оставить пустыми, установщик сгенерирует их и покажет один раз в конце установки. `MAX_BOT_TOKEN` можно оставить пустым и заполнить позже в `/opt/max-hr-recruitment-bot/.env`.
